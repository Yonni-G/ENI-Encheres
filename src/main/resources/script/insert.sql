...
IF EXISTS (SELECT * FROM sys.foreign_keys WHERE name = 'encheres_articles_vendus_fk')
ALTER TABLE ENCHERES DROP CONSTRAINT encheres_articles_vendus_fk;

IF EXISTS (SELECT * FROM sys.foreign_keys WHERE name = 'encheres_utilisateur_fk')
ALTER TABLE ARTICLES_VENDUS DROP CONSTRAINT encheres_utilisateur_fk;

IF EXISTS (SELECT * FROM sys.foreign_keys WHERE name = 'articles_vendus_categories_fk')
ALTER TABLE ARTICLES_VENDUS DROP CONSTRAINT articles_vendus_categories_fk;

IF EXISTS (SELECT * FROM sys.foreign_keys WHERE name = 'ventes_utilisateur_fk')
ALTER TABLE ARTICLES_VENDUS DROP CONSTRAINT ventes_utilisateur_fk;

IF EXISTS (SELECT * FROM sys.foreign_keys WHERE name = 'retraits_articles_vendus_fk')
ALTER TABLE RETRAITS DROP CONSTRAINT retraits_articles_vendus_fk;

-- Supprimer les clés primaires si elles existent
IF EXISTS (SELECT * FROM sys.key_constraints WHERE name = 'categorie_pk')
ALTER TABLE CATEGORIES DROP CONSTRAINT categorie_pk;

IF EXISTS (SELECT * FROM sys.key_constraints WHERE name = 'enchere_pk')
ALTER TABLE ENCHERES DROP CONSTRAINT enchere_pk;

IF EXISTS (SELECT * FROM sys.key_constraints WHERE name = 'retrait_pk')
ALTER TABLE RETRAITS DROP CONSTRAINT retrait_pk;

IF EXISTS (SELECT * FROM sys.key_constraints WHERE name = 'utilisateur_pk')
ALTER TABLE UTILISATEURS DROP CONSTRAINT utilisateur_pk;

IF EXISTS (SELECT * FROM sys.key_constraints WHERE name = 'articles_vendus_pk')
ALTER TABLE ARTICLES_VENDUS DROP CONSTRAINT articles_vendus_pk;

-- Supprimer les tables si elles existent
IF EXISTS (SELECT * FROM sys.objects WHERE type = 'U' AND name = 'ENCHERES')
    DROP TABLE ENCHERES;

IF EXISTS (SELECT * FROM sys.objects WHERE type = 'U' AND name = 'RETRAITS')
    DROP TABLE RETRAITS;

IF EXISTS (SELECT * FROM sys.objects WHERE type = 'U' AND name = 'ARTICLES_VENDUS')
    DROP TABLE ARTICLES_VENDUS;

IF EXISTS (SELECT * FROM sys.objects WHERE type = 'U' AND name = 'UTILISATEURS')
...
VALUES
    ...
    ('fbernard', 'Bernard', 'François', 'fbernard@example.com', '0698765432', '18 Boulevard Haussmann', '67000', 'Strasbourg', '{bcrypt}$2a$10$RUP813cgJ0sLL2W5VqGBvuXyySMACT.DpKNGsVF41DEiS6MB9g8RW', 300, 0),
    ('aline', 'Durand', 'Aline', 'aline@example.com', '0609876543', '3 Rue des Fleurs', '76000', 'Rouen', '{bcrypt}$2a$10$RUP813cgJ0sLL2W5VqGBvuXyySMACT.DpKNGsVF41DEiS6MB9g8RW', 80, 0),
    ('chris', 'Leclerc', 'Christophe', 'chris@example.com', '0623456789', '25 Allée des Chênes', '34000', 'Montpellier', '{bcrypt}$2a$10$RUP813cgJ0sLL2W5VqGBvuXyySMACT.DpKNGsVF41DEiS6MB9g8RW', 500, 0),
    ('lucieB', 'Blanc', 'Lucie', 'lucieB@example.com', '0634567890', '8 Rue Pasteur', '80000', 'Amiens', '{bcrypt}$2a$10$RUP813cgJ0sLL2W5VqGBvuXyySMACT.DpKNGsVF41DEiS6MB9g8RW4', 120, 0);
...
VALUES
    (3, '123 rue de Paris', '75001', 'Paris'),  -- Retrait pour Ordinateur Portable
